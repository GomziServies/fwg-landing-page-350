...
const HeroSection = () => {
  ...
  const validateForm = () => {
    if (!userDetails.name || !userDetails.email || !userDetails.mobile || 
        !userDetails.goal || !userDetails.experience || !userDetails.preferredTimeSlot) {
    ...
    try {
      const options = {
        ...
        handler: async function (response) {
          try {
              const result = await axios.post(`${apiConfig.BASE_URL}/public/v1/guest-payment/custom-payment-350`, {
              ...
              preferred_time_slot: userDetails.preferredTimeSlot
  ...
  <div className="grid grid-cols-1  md:grid-cols-3 gap-6 items-start">
    ...
    <div className="md:col-span-1 flex flex-col items-center bg-gradient-to-b from-green-50 to-green-100 rounded-2xl p-6 shadow-sm">
      ...
      <p className="mt-2 text-sm text-gray-600 text-center leading-relaxed">
        Fill a few quick details and secure your monthly training seat with our
        expert coaches.
      </p>
    </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Select
            ...
            onChange={(value) =>
              handleInputChange({
                target: { name: "preferredTimeSlot", value },
              })
            }
            options={timeSlots}
            inputClassName="h-11 px-3 rounded-lg border border-gray-300 focus:border-green-500 focus:ring-1 focus:ring-green-500"
    ...
    <Modal isOpen={isThankYouModalOpen} onClose={() => setIsThankYouModalOpen(false)} title="Thank you!">
      <div className="flex flex-col items-center text-center gap-4">
        ...
        <p className="text-sm text-gray-600 max-w-sm">Payment received. Our team will reach out to schedule your first session and share onboarding details.</p>
